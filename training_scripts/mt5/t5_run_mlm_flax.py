...
class TrainingArguments:
    ...
    trial: int = field(default=0, metadata={"help": "The number of a training run"})
    ...
    total_steps: int = field(default=None, metadata={"help": "Stop training after X steps."})
    seed: int = field(default=None, metadata={
        ...
        if self.dataset_name is None and self.train_file is None and self.validation_file is None:
            ...
            if self.train_file is not None:
                ...
                assert extension in ["csv", "json",
                                     "txt"], "`train_file` should be a csv, a json or a txt file."
...
class EarlyStopping:
    # class taken from NegBERT by Aditya Khandelwal
    """Early stops the training if validation loss doesn't improve after a given patience. Patience here is not measured in the numbeer of epochs but in the number of eval_steps. If eval_steps=10 and patience is set to 5, then patience is 50 steps long (5 evaluation cycles)"""
    ...
    def __call__(self, score, model, params, output_dir):

        if self.best_score is None:
            # set new score and save model
            self.best_score = score
            self.save_checkpoint(score, model, output_dir, params)
        elif score > self.best_score:

            self.write = False
            
        else:
            # if the score is lower (=better) than before, set new score, save model and cancel patience by resetting the epoch counter to 0
            self.best_score = score
            self.save_checkpoint(score, model, output_dir, params)
            self.counter = 0

    def save_checkpoint(self, val_loss, model, output_dir, params):
...
def compute_input_and_target_lengths(inputs_length, noise_density, mean_noise_span_length):
    ...
    # print("TARGET_LENGTH:", targets_length)
...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        datasets = load_dataset(data_args.dataset_name,
                                data_args.dataset_config_name, cache_dir=model_args.cache_dir)
    ...
    print("\n TOKEN LENGTHS: max_seq_length=", max_seq_length,
    ...
    if model_args.model_name_or_path:
        model = FlaxT5ForConditionalGeneration.from_pretrained(
            model_args.model_name_or_path, config=config, seed=training_args.seed, dtype=getattr(
    ...
    num_epochs = training_args.total_steps // (len(tokenized_datasets["train"]) // train_batch_size)

    
    num_train_steps = len(tokenized_datasets["train"]) // train_batch_size * num_epochs

    if num_train_steps < training_args.total_steps:
    ...
    for epoch in epochs:
        ...
        print("train_batch_idx", len(train_batch_idx))
        ...
        for step, batch_idx in enumerate(train_batch_idx):
            ...
            if cur_step % training_args.eval_steps == 0 and cur_step > 0:
                ...
                if jax.process_index() == 0:
                    ...
                    early_stopping(eval_metrics["loss"], model, params, training_args.output_dir)

                    if early_stopping.write:
                        eval_metrics = {f"eval_{metric_name}": value for metric_name,
                                        value in eval_metrics.items()}
                        eval_metrics["step"] = cur_step
                        eval_metrics["seed"] = training_args.seed
                        print(eval_metrics)

                        path = os.path.join(training_args.output_dir,
                                            f"eval_results_{training_args.trial}.json")
                        with open(path, "w") as f:
                            json.dump(eval_metrics, f, indent=4, sort_keys=True)

                if cur_step == training_args.total_steps:
                    break
