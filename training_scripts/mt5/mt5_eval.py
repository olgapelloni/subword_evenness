...
class TrainingArguments:
    ...
    seed: int = field(default=55, metadata={
...
class DataTrainingArguments:
    ...
    language: Optional[str] = field(
        default=None, metadata={"help": "Languag of the test file"})
...
def main():
    ...
    # num_epochs = 2000 // (len(tokenized_datasets["train"]) // train_batch_size)
    #
    ...
    if training_args.do_eval:
        ...
        eval_metrics = []
        for i, batch_idx in enumerate(tqdm(eval_batch_idx, desc="AFTER TRAIN Evaluating ...", position=2)):
        ...
        if jax.process_index() == 0:
            eval_metrics["lang"] = data_args.language
            eval_metrics["model"] = model_args.model_name_or_path
            eval_metrics = {f"eval_{metric_name}": value for metric_name,
                            value in eval_metrics.items()}
            path = os.path.join(training_args.output_dir, f"{data_args.language}.json")
            with open(path, "a") as f:
                json.dump(eval_metrics, f, sort_keys=True)
                f.write("\n")
